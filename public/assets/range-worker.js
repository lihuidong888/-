self.onmessage = function (e) {
    let rectPoints = e.data.rectPoints;
    ...
    for (let i = 0; i < rectPoints.length; i++) {
        //判断bbox的点是否在有效范围内
        if (isPointInPolygon(rectPoints[i], points, indices)) {
            num++;
            newArr.push(rectPoints[i]);
        }
    }
    console.log("地形网格数量：" + num);
    this.postMessage({ points: newArr });
    this.close();
};
...
function isPointInTriangle(P, [A, B, C]) {
    let PA = Cartesian3.subtract(A, P);
    let PB = Cartesian3.subtract(B, P);
    let PC = Cartesian3.subtract(C, P);
    let n1 = Cartesian3.cross(PA, PB).z;
    let n2 = Cartesian3.cross(PB, PC).z;
    let n3 = Cartesian3.cross(PC, PA).z;
    let flag = n1 * n2 >= 0 && n1 * n3 >= 0 && n2 * n3 >= 0;
    return flag;
}

function isPointInPolygon(target, pointArray, indices) {
...
function Cartesian3(x = 0, y = 0, z = 0) {
    this.x = x;
    this.y = y;
    this.z = z;
}
